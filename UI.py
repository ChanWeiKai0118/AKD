...
def save_to_gsheet(data):
    ...
    # 在 A 欄插入 id_no 公式
    row[0] = f'=IF(ROW()=2, 1, IF(COUNTIF(B$2:B{last_row-1}, B{last_row}) = 0, MAX(A$2:A{last_row-1}) + 1, IF(OR(H{last_row}<INDEX(H$2:H{last_row-1}, MATCH(B{last_row}, B$2:B{last_row-1}, 0)), I{last_row}<INDEX(I$2:I{last_row-1}, MATCH(B{last_row}, B$2:B{last_row-1}, 0)))), MAX(A$2:A{last_row-1}) + 1, INDEX(A$2:A{last_row-1}, MATCH(B{last_row}, B$2:B{last_row-1}, 0))))))'
